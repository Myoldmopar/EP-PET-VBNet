...
namespace MatrixLibrary
{
	...
	public class Matrix
	{
		...
		#region "    My version of solving a set of equations    "
		/// <summary>
		/// Solves a set of n linear equations A.X = B, and returns
		/// X[#coefficients,1], where A is [#points,#coefficients] and B is [#points,1]. 
		/// In case of an error the error is raised as an exception. 
		/// Note: This method is based on the 'LU Decomposition and Its Applications'
		/// section of Numerical Recipes in C by William H. Press,
		/// Saul A. Teukolsky, William T. Vetterling and Brian P. Flannery,
		/// University of Cambridge Press 1992.
		/// </summary>
		/// <param name="F">The array 'A' on the left side of the equations A.X = B</param>
		/// <param name="Y">The array 'B' on the right side of the equations A.X = B</param>
		/// <returns>Array 'X' in the system of equations A.X = B</returns>
		public static Matrix SolveEqSet(Matrix F, Matrix Y)
		{
			Matrix F_trans  = MatrixLibrary.Matrix.Transpose(F);
			Matrix F_trans_F  = MatrixLibrary.Matrix.Multiply(F_trans, F);
			Matrix F_trans_F_inv  = MatrixLibrary.Matrix.Inverse(F_trans_F);
			Matrix F_trans_Y  = MatrixLibrary.Matrix.Multiply(F_trans, Y);
			Matrix C = MatrixLibrary.Matrix.Multiply(F_trans_F_inv, F_trans_Y);
			return C;
		}
		#endregion
        ...
        /// <summary>
        /// Returns the object base class hash code
        /// </summary>
        /// <returns></returns>
